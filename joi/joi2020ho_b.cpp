...
int ans=200001;
void dfs(const string& target,const string& S,int a,int titr,int sitr,bool isedge){
    if(titr==target.size()){
        chmin(ans,a);
        return;
    }
    if(sitr==S.size()) return;
    if(S[sitr]==target[titr]) dfs(target,S,a,titr+1,sitr+1,false);
    if(isedge){
        dfs(target,S,a,titr,sitr+1,isedge);
    }else{
        dfs(target,S,a+1,titr,sitr+1,isedge);
    }
}
int main(){
    int N,K; cin>>N>>K;
    string S; cin>>S;
    string T;
    rep(i,K) T.push_back('J');
    rep(i,K) T.push_back('O');
    rep(i,K) T.push_back('I');
    dfs(T,S,0,0,0,true);
    if(ans==200001){
        print(-1);
        return 0;
