...
void dfs(vvi& Graph,vi& T,vector<bool>& seen,int npos){
    seen[npos]=true;
    for(int& i:Graph[npos]){
        if(seen[i]) continue;
        T[i]=npos;
        dfs(Graph,T,seen,i);
    }
...
int main(){
   ...
   xpath.push_back(x);
   ypath.push_back(y);
   while(xpath.back()!=0) xpath.push_back(tree[xpath.back()]);
   while(ypath.back()!=0) ypath.push_back(tree[ypath.back()]);
   int a;
   while(xpath.back()==ypath.back()){
    a=xpath.back();
    xpath.pop_back();
    ypath.pop_back();
   }
    reverse(ypath.begin(), ypath.end());
   for(int& i:xpath) cout<<i+1<<" ";
   cout<<a+1<<" ";
   for(int& i:ypath) cout<<i+1<<" ";
   cout<<endl;
   return 0;
}