...
void dfs(vb& seen,vvll& graph,ll npos){
    seen[npos]=true;
    for(const ll& i:graph[npos]){
        if(seen[i]) continue;
        dfs(seen,graph,i);
    }
}
int main(){
    ...
    vvll G(N);
    rep(i,N){
        if((a-x[i])*(a-x[i])+(b-y[i])*(b-y[i])==r[i]*r[i]) s=i;
        if((c-x[i])*(c-x[i])+(d-y[i])*(d-y[i])==r[i]*r[i]) t=i;
        reps(j,i+1,N){
            ll dist=(x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]);
            if(dist>=(r[i]-r[j])*(r[i]-r[j])&&dist<=(r[i]+r[j])*(r[i]+r[j])){
                G[i].pb(j);
                G[j].pb(i);
            }
        }
